...
# Try modifying the bayes freq for unknown by specificity - highly specific phenotypes will have higher frequency
# if a phenotype is very common - it will count as very low evidence for the disorder - but will still show up as unknown in the table
...
# Read the test table
load("~/Projects/phenotype_questionaire_202203/data/preprocessed_data.RData")
...
exclude_unknown_frequency_from_questions <- TRUE
message(glue("SETUP: defined frequency for unknown phenotypes: {bayes_freq_for_unknown}"))
message(glue("SETUP: phenotypes with unknown frequency will be skipped: {exclude_unknown_frequency_from_questions}"))
      ...
      mainPanel(
        ...
        fluidRow(
          column(
            10,
            div(
              style = "max-width: 500px;", # You can adjust the max-width value as needed
              sliderInput(
                inputId = "analysis_depth",
                label = strong("Phenotypes included in questionaire"),
                value = 0.1, max = 1, min = 0, step = 0.01
              ),
              div(
                style = "display: flex; justify-content: space-between; font-size:10px;",
                tags$span("(Phenotypes from unlikely disorders included)"),
                tags$span("(Phenotypes from unlikely disorders excluded)")
              )
            )
          )
        ),
        ...
        fluidRow(
          column(
            ...
            div(
              style = "max-width: 500px;", # You can adjust the max-width value as needed
              sliderTextInput(
                inputId = "abs_change_thresh",
                label = strong("Mean absolute change in likelihood"),
                choices = c(10^seq(-3, -7, -1)), selected = 10^-4, grid = T, hide_min_max = F
              ),
              div(
                style = "display: flex; justify-content: space-between; font-size:10px;",
...
server <- function(input, output, session) {
  ...
  previous_likelihoods <- reactiveVal()
  ...
  all_phenos_data <- reactive({
    ...
    if (is.null(main_phenos) & is.null(selected_panelapp_panels)) {
      return(NULL)
    }

    # If generating panel using panelapp panels
    if (!is.null(selected_panelapp_panels)) {
      panelapp_genes <- panelapp %>%
    ...
    all_phenos_from_disorders_with_main_phenos <- complete_grid %>%
      left_join(all_phenos_from_disorders_with_main_phenos) %>%
  ...
  hpos_by_count_reactive <- reactive({
    message("REACTIVE: hpos_by_count_reactive")
    phenos_data <- all_phenos_data()
    mean_change <- 0
    if (is.null(phenos_data)) {
      return(NULL)
    }
    message("DEBUG: excluding hpos according to previous answers..")
    # Exclude all ancestors of phenos that the user said are present or maybe present in the patient
    excluded_due_to_addition <- c(
      phenos_data$main_phenos,
    ...
    likelihoods_before_update <- as.numeric(isolate(previous_likelihoods()))
    message(glue("INFO: mean likelihoods before update {mean(likelihoods_before_update)}"))

    disorders_with_likelihood <- update_disorders_likelihood_bayes()

    likelihoods_after_update <- as.numeric(disorders_with_likelihood$likelihood)
    ...
    message(glue("INFO: likelihood thresh: {likelihood_thresh}"))
    disorders_remaining <- disorders_with_likelihood %>%
      filter(likelihood >= likelihood_thresh) %>%
      pull(disease_id)
    unlikely_disorders <- disorders_with_likelihood %>%
      filter(likelihood < likelihood_thresh) %>%
      pull(disease_id)
    if (mean_change != 0 & mean_change < input$abs_change_thresh) {
      message("TOO LOW")
      unlikely_disorders <- disorders_with_likelihood$disease_id
      disorders_remaining <- c()
    }

    message(glue("INFO: There are currently {length(unlikely_disorders)}/{nrow(disorders_with_likelihood)} unlikely disorders.."))
    ...
    hpos_from_disorders_that_are_not_in_main <- phenos_data$all_phenos_from_disorders_with_main_phenos %>%
      ...
      filter(!(hpo_id_name %in% c(
        excluded_due_to_addition_ancestors,
        excluded_due_to_addition_descendants,
        excluded_due_to_rejection_descendants
      )))
    if (exclude_unknown_frequency_from_questions) {
      hpos_from_disorders_that_are_not_in_main <- hpos_from_disorders_that_are_not_in_main %>% filter(frequency_cat != "unknown")
    }
    print(nrow(hpos_from_disorders_that_are_not_in_main))
    message("DEBUG: summarizing descendants..")
    final_descendants <- as.data.table(hpos_from_disorders_that_are_not_in_main %>%
      ...
      summarize(
        final_desc = paste0(unique(ancestor_of)[order(unique(ancestor_of))], collapse = "<br>"),
        num_final_desc = length(unique(ancestor_of))
      ))

    # final_descendants<-NULL #
    message("DEBUG: counting remaining hpos..")
  ...
  observe({
    ...
    output$hpos_by_count_table <- renderDataTable({
      hpos_by_count_reactive_output$hpos_by_count
    })
    num_of_phenos_that_are_obligated <- nrow(hpos_by_count)
    message(glue("DEBUG: remaining HPOs: {num_of_phenos_that_are_obligated}"))
  ...
  update_disorders_likelihood_bayes <- function() {
    ...
    combined_updates <- phenotype_disease_table %>%
      left_join(priors) %>%
      ...
      mutate(
        product_selected = replace_na(product_selected, 1),
    ...
    combined_updates <- combined_updates %>%
      mutate(
        likelihood = updated_probability / sum(updated_probability),
      ...
      relocate(disease_name)
    ecdf_values <- ecdf(combined_updates$likelihood)
    previous_likelihoods(combined_updates$likelihood)
  ...
  panel_genes_reactive <- reactive({
    ...
    if (!is.null(selected_panelapp_panels)) {
      ...
      panelapp_genes_not_in_disorder_to_gene_table <- panelapp %>%
        ...
        mutate(
          disease_id = glue("PANELAPP:{panel_id}"),
    ...
    if (is.null(panelapp_genes_not_in_disorder_to_gene_table)) {
      panel_genes_table <- disorder_to_gene_for_table %>%
        ...
        slice_max(n = 1, with_ties = F, order_by = frequency_bayes) %>%
        ungroup() %>%
        group_by(gene_symbol) %>%
        summarize(
          disorders = paste0(unique(disease_id_name), collapse = " | "),
